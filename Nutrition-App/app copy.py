...
st.set_page_config(page_title = "Job Buddy AI")
st.header("Job Buddy.AI")

subCustomPrompt = st.button("Personal Prompt?")

if subCustomPrompt:
    custom_prompt = st.text_area("Custom Prompt: ", key="custom-prompt")
...
if subPcentMatch:
    if uploaded_file is not None:
        ...
        response= get_gemini_response(pcent_match_prompt,pdf_content,input_text.lower()) if subCustomPrompt == False else get_gemini_response(custom_prompt,pdf_content,input_text.lower())
    ...
    if uploaded_file is not None:
        ...
        response= get_gemini_response(about_resume_prompt,pdf_content,input_text.lower()) if subCustomPrompt == False else get_gemini_response(about_resume_prompt,pdf_content,input_text.lower())
        st.subheader("The Response is")
        st.write(response)
    else:
        st.write("Please upload the resume")
    ...
    if uploaded_file is not None:
        ...
        response= get_gemini_response(improve_skills_prompt,pdf_content,input_text.lower()) if subCustomPrompt == False else get_gemini_response(improve_skills_prompt,pdf_content,input_text.lower())
        st.subheader("The Response is")
        st.write(response)
    else:
        st.write("Please upload the resume")
